    ...
    return (
		<div>
		  ...
		  <button
			...
			className={`bg-${
			  sortBy === "name" ? "blue" : "gray"
			}-500 text-white mx-2 px-4 py-2 rounded`}
		  >
			Name
		  </button>
		  ...
		  <button
			...
			className={`bg-${
			  sortBy === "grouped-category" ? "blue" : "gray"
			}-500 text-white mx-2 px-4 py-2 rounded`}
		  >
			Grouped Category
		  </button>
	  ...
	  <div key={item.isCategoryHeading ? `heading-${item.category}-${index}` : `item-${item.id}`}>
